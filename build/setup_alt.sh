...
# AtlasCore when sourced from either BASH or ZSH.
#
...
ENV_VAR=AtlasCore_SET_UP
if [ "$*" = "extonly" ]; then
    ENV_VAR=AtlasCore_EXTONLY_SET_UP
elif [ "$*" = "relonly" ]; then
    ENV_VAR=AtlasCore_RELONLY_SET_UP
fi
...
if [ "x${ENV_VAR_VALUE}" = "x" ]; then
    ...
    export AtlasCore_DIR=$(cd ${thisdir};pwd)
    unset thisdir

    # Extract directory names relevant for the rest of the operations:
    export AtlasCore_PLATFORM=$(basename ${AtlasCore_DIR})
    export AtlasCore_VERSION=$(basename $(dirname $(dirname ${AtlasCore_DIR})))

    # Base projects used for the build of AtlasCore:
    AtlasCore_BASEPROJECTS=" DetCommon"

    # If the project has base projects, set up those first. First, we just set
    # up the external environment of the base projects:
    if [ "$*" != "relonly" ]; then
        for project in ${AtlasCore_BASEPROJECTS}; do
            # Strip off the whitespaces from the project name:
            p="$(echo ${project} | xargs)"
            # Name of the setup file in the base release:
            fname=${AtlasCore_DIR}/../../../../${p}/${AtlasCore_VERSION}/InstallArea/${AtlasCore_PLATFORM}/setup.sh
            if [ -f ${fname} ]; then
                . ${fname} extonly
            fi
            unset fname p
    ...
    if [ "$*" != "relonly" ]; then
        ...
        if [ -f ${AtlasCore_DIR}/env_setup.sh ]; then
            . ${AtlasCore_DIR}/env_setup.sh
        elif [ -f ${AtlasCore_DIR}/lcg_setup.sh ]; then
            . ${AtlasCore_DIR}/lcg_setup.sh
        fi
    fi
    ...
    if [ "$*" != "extonly" ]; then
        ...
        if [ -z "${CMAKE_PREFIX_PATH}" ]; then
            export CMAKE_PREFIX_PATH=${AtlasCore_DIR}:
        else
            export CMAKE_PREFIX_PATH=${AtlasCore_DIR}:${CMAKE_PREFIX_PATH}:
        fi

        # Set up the binary path(s):
        if [ -z "${PATH}" ]; then
            export PATH=${AtlasCore_DIR}/bin:
        else
            export PATH=${AtlasCore_DIR}/bin:${PATH}:
        fi
        export PATH=${AtlasCore_DIR}/share:${PATH}:

        # Set up the library path(s):
        if [ -z "${LD_LIBRARY_PATH}" ]; then
            export LD_LIBRARY_PATH=${AtlasCore_DIR}/lib:
        else
            export LD_LIBRARY_PATH=${AtlasCore_DIR}/lib:${LD_LIBRARY_PATH}:
        fi
        if [ -z "${DYLD_LIBRARY_PATH}" ]; then
            export DYLD_LIBRARY_PATH=${AtlasCore_DIR}/lib:
        else
            export DYLD_LIBRARY_PATH=${AtlasCore_DIR}/lib:${DYLD_LIBRARY_PATH}:
        fi

        # Set up the python path(s):
        if [ -z "${PYTHONPATH}" ]; then
            export PYTHONPATH=${AtlasCore_DIR}/lib:
        else
            export PYTHONPATH=${AtlasCore_DIR}/lib:${PYTHONPATH}:
        fi
        export PYTHONPATH=${AtlasCore_DIR}/python:${PYTHONPATH}:

        # Set up the jobOptions path:
        if [ -z "${JOBOPTSEARCHPATH}" ]; then
            export JOBOPTSEARCHPATH=${AtlasCore_DIR}/jobOptions:
        else
            export JOBOPTSEARCHPATH=${AtlasCore_DIR}/jobOptions:${JOBOPTSEARCHPATH}:
        fi

        # Set up the data path(s):
        if [ -z "${DATAPATH}" ]; then
            export DATAPATH=${AtlasCore_DIR}/share:
        else
            export DATAPATH=${AtlasCore_DIR}/share:${DATAPATH}:
        fi
        export DATAPATH=${AtlasCore_DIR}/data:${DATAPATH}:
        if [ -z "${CALIBPATH}" ]; then
            export CALIBPATH=${AtlasCore_DIR}/share:
        else
            export CALIBPATH=${AtlasCore_DIR}/share:${CALIBPATH}:
        fi
        export CALIBPATH=${AtlasCore_DIR}/data:${CALIBPATH}:

        # Set up the include path:
        if [ -z "${ROOT_INCLUDE_PATH}" ]; then
            export ROOT_INCLUDE_PATH=${AtlasCore_DIR}/include:
        else
            export ROOT_INCLUDE_PATH=${AtlasCore_DIR}/include:${ROOT_INCLUDE_PATH}:
        fi

        # Set up the XML path:
        if [ -z "${XMLPATH}" ]; then
            export XMLPATH=${AtlasCore_DIR}/XML:
        else
            export XMLPATH=${AtlasCore_DIR}/XML:${XMLPATH}:
        fi
...
# Clean up:
unset ENV_VAR
if [ ! -z "${AtlasCore_UNSETSHWORDSPLIT}" ]; then
    unsetopt shwordsplit
    unset AtlasCore_UNSETSHWORDSPLIT
fi
